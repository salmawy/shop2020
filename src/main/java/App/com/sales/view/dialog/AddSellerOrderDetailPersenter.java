...
private void packageNumberTracker() {
    ...
    if(productType.getSelectionModel().getSelectedItem().getValue()==3){
           String weight=grossWeight.getText();
	...
	private void loadCustomerOrders() {
		...
		int storeId=store_cb.getSelectionModel().getSelectedItem().getValue();

	List result=this.getSalesService().getAllCustomersOrdersTags(this.getSeason().getId(), this.getFridage().getId(), productId, storeId);
